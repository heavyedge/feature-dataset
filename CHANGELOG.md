# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [1.1.0.post0] - 2026-08-29

### Added

**v1**

- `examples/v1/phi.ipynb` now displayes class probability of an example profile and its projection.
- `examples/v1/shape_features.ipynb` now shows box-whisker plot of shape features from all profiles.
- `examples/v1/shape_loss.ipynb` is added.
- Bayesian optimization benchamark now uses all profiles.

### Fixed

**v1**

- `examples/v1/phi.ipynb` now displayes example profiles of phi distribution in correct order.

## [1.1.0] - 2026-08-18

**v1**

- Dataset: `heavyedge/profiles:v1.0.0`
- Classifier:
  - `heavyedge-classify==1.4.0`
  - `jeesoo9595/classifier-v1:v1.0.0`
- Feature extractor: `heavyedge-features==1.2.0`
- Output features:
  - `H`
  - `b`
  - `phi`
  - `phi_0`
  - `phi_1`
  - `phi_2`
  - `phi_3`
  - `phi_4`
  - `phi_5`
  - `phi_6`

## [1.0.0] - 2026-08-18

**v1**

- Dataset: `heavyedge/profiles:v1.0.0`
- Classifier:
  - `heavyedge-classify==1.4.0`
  - `jeesoo9595/classifier-v1:v1.0.0`
- Feature extractor: `heavyedge-features==1.1.0`
- Output features:
  - `H`
  - `b`
  - `phi`
